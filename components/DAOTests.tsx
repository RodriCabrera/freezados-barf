...
export function DAOTests() {
    ...
    useEffect(() => {
        ...
        users.insertOne({name: 'Roberto Gomez Bolaños'}).then(res => {
            if(res) {
                setUserId(res)
                users.getById(res).then(newUser => console.log("\nUsuario agregado", newUser))
            }
        })
    }, [])
    ...
    useEffect(() => {
        ...
        ubications.insertOne(
            {user_id: userId, description: 'Freezer de casa', isFreezer: true, name: 'Freezer', ubication: 'Casa'}
        ).then(res => {
            if (res) { 
                setUbicationId(res)
                ubications.getById(res).then(newUb => console.log("\nUbicacion agregada: ", newUb))
            }
        })
        foods.insertOne(
            {name: 'Pechuga de pollo', species: 'AVIAR', user_id: userId, description: 'Pechuga del coto'}
        ).then(res => {
            if (res) {
                setFoodId(res)
                foods.getById(res).then(newFood => console.log("\nAlimento agregado: ", newFood))
            }
        })
    }, [userId])
    ...
    useEffect(() => {
        ...
        entries.insertOne({
            ...
            if (res) {
                setEntryId(res)
                entries.getById(res).then(newEntry => console.log("\nNuevo elemento guardado: ", newEntry))
            }
        })
